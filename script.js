const conteudos = [
    { titulo: "La Casa de Papel", capaID: "10ZcHAcPOC0TmNU1qBi31LD7cZoNNdj6G", tipo: "serie", genero: "Séries", episodios: [{ nome: "Temporada 1 - Ep 01", videoID: "1JtZ4dIdU3svU8lpPnAt5Y-SMb41vhX4D" }] },
    { titulo: "A Má Mãe (2023)", capaID: "1_NY-gbUM21gbOdsBf56zVjNtm8KUDYoi", tipo: "dorama", genero: "Séries", episodios: [{ nome: "Episódio 01", videoID: "1_tOC-zRf2hIDxrmZiHd3gpImrj0yIWzV" }] },
    ...
    { titulo: "After 1 (2019)", capaID: "1SM2PN1hPWL0Z_mQRFTVvMoPBwtqD9rtB", videoID: "1RZE1S_UCi9DA-Q-9DZIKTyQmuBNSPHQ_", tipo: "filme", genero: "Romance" },
...
function validarChave() {
    ...
    const senha = prompt("Chave VOLTTI:");
...
function renderizar(lista) {
    ...
    generos.forEach(gen => {
        ...
        lista.filter(i => i.genero === gen).forEach(item => {
            ...
            card.onclick = () => { 
                if(validarChave()) { 
                    if(item.episodios) { abrirMenuSerie(item); } 
                    else { darPlay(item.videoID, item.titulo); } 
                } 
            };
...
function abrirMenuSerie(serie) {
    ...
    listaEpsContainer.innerHTML = `<div style="color:#8B5CF6; font-size:11px; margin-bottom:10px;">EPISÓDIOS DISPONÍVEIS:</div>`;
    serie.episodios.forEach(ep => {
        const btn = document.createElement('button');
        btn.innerText = ep.nome; btn.className = "btn-episodio";
        btn.onclick = () => darPlay(ep.videoID, ep.nome);
...
function executarBusca() {
    ...
    renderizar(conteudos.filter(i => i.titulo.toLowerCase().includes(termo)));
}
