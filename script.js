const conteudos = [
    ...
    { titulo: "A Má Mãe (2023)", capaID: "1_NY-gbUM21gbOdsBf56zVjNtm8KUDYoi", tipo: "dorama", genero: "Dorama", episodios: [{ nome: "Episódio 01", videoID: "1_tOC-zRf2hIDxrmZiHd3gpImrj0yIWzV" }] }
];
...
function renderizar(lista) {
    ...
    generos.forEach(gen => {
        ...
        lista.filter(i => i.genero === gen).forEach(item => {
            ...
            card.onclick = () => { if(validarChave()) { if(item.episodios) { videoTitle.innerText = item.titulo; modal.style.display = "flex"; gerarLista(item); } else { darPlay(item.videoID, item.titulo); } } };
